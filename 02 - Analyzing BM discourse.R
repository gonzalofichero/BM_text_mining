...
clean_bm <- tm_map(clean_bm, content_transformer(gsub), pattern = "years", replacement = "year", fixed=TRUE)
clean_bm <- tm_map(clean_bm, content_transformer(gsub), pattern = "teachers", replacement = "teacher", fixed=TRUE)
clean_bm <- tm_map(clean_bm, content_transformer(gsub), pattern = "teacher", replacement = "teach", fixed=TRUE)
clean_bm <- tm_map(clean_bm, content_transformer(gsub), pattern = "teaching", replacement = "teach", fixed=TRUE)
clean_bm <- tm_map(clean_bm, content_transformer(gsub), pattern = "schooling", replacement = "school", fixed=TRUE)
clean_bm <- tm_map(clean_bm, content_transformer(gsub), pattern = "schools", replacement = "school", fixed=TRUE)
clean_bm <- tm_map(clean_bm, content_transformer(gsub), pattern = "systems", replacement = "system", fixed=TRUE)
clean_bm <- tm_map(clean_bm, content_transformer(gsub), pattern = "students", replacement = "student", fixed=TRUE)
...
#rect.hclust(fit_14, k=4, border="red") # draw dendogram with red borders around the 6 clusters 
...
word_associate(
  bm_14$.,
  match.string = c("educ"), # palabra clave!
